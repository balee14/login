...
public class Aes256 {
    ...
    System.out.print("AES Static Tablesn");
        System.out.print("S[] = n"); for(i=0;i<16;i++) { for(j=0;j<16;j++) System.out.print(AES256Util.toHEX1(S[i*16+j])+", "); System.out.println();}
        System.out.print("Si[] = n"); for(i=0;i<16;i++) { for(j=0;j<16;j++) System.out.print(AES256Util.toHEX1(Si[i*16+j])+", "); System.out.println();}
    System.out.print("rcon[] = n"); for(i=0;i<5;i++) {for(j=0;j<6;j++) System.out.print(AES256Util.toHEX1(rcon[i*6+j])+", "); System.out.println();}
    System.out.print("log[] = n"); for(i=0;i<32;i++) {for(j=0;j<8;j++) System.out.print(AES256Util.toHEX1(log[i*8+j])+", "); System.out.println();}
    System.out.print("alog[] = n"); for(i=0;i<32;i++) {for(j=0;j<8;j++) System.out.print(AES256Util.toHEX1(alog[i*8+j])+", "); System.out.println();}
    }
    ...
    if (traceLevel > 0) traceInfo = "encryptAES(" + AES256Util.toHEX1(plain) + ")";
    ...
    if (traceLevel > 2)
        traceInfo += "n  R0 (Key = "+AES256Util.toHEX1(Ker)+")ntAK = "+AES256Util.toHEX1(a);
    else if (traceLevel > 1)
        traceInfo += "n  R0 (Key = "+AES256Util.toHEX1(Ker)+")t = "+AES256Util.toHEX1(a);

    // for each round except last, apply round transforms
    for (int r = 1; r < numRounds; r++) {
            Ker = Ke[r];            // get session keys for this round
        if (traceLevel > 1)    traceInfo += "n  R"+r+" (Key = "+AES256Util.toHEX1(Ker)+")t";

            // SubBytes(state) into ta using S-Box S
        for (i = 0; i < BLOCK_SIZE; i++) ta[i] = S[a[i] & 0xFF];
        if (traceLevel > 2)    traceInfo += "ntSB = "+AES256Util.toHEX1(ta);
        ...
        if (traceLevel > 2)    traceInfo += "ntSR = "+AES256Util.toHEX1(a);
    ...
    for (i = 0; i < BLOCK_SIZE; i++)    a[i] = (byte)(ta[i] ^ Ker[i]);
    if (traceLevel > 2)    traceInfo += "ntAK";
    if (traceLevel > 1)    traceInfo += " = "+AES256Util.toHEX1(a)+"n";
    if (traceLevel > 0)    traceInfo += " = "+AES256Util.toHEX1(a)+"n";
    return (a);
    }
    ...
    traceInfo = "";                // reset trace info
    if (traceLevel > 0) traceInfo = "decryptAES(" + AES256Util.toHEX1(cipher) + ")";
    ...
    if (traceLevel > 2)
        traceInfo += "n  R0 (Key = "+AES256Util.toHEX1(Kdr)+")nt AK = "+AES256Util.toHEX1(a);
    else if (traceLevel > 1)
        traceInfo += "n  R0 (Key = "+AES256Util.toHEX1(Kdr)+")t = "+AES256Util.toHEX1(a);

    // for each round except last, apply round transforms
        for (int r = 1; r < numRounds; r++) {
            Kdr = Kd[r];            // get session keys for this round
        if (traceLevel > 1)    traceInfo += "n  R"+r+" (Key = "+AES256Util.toHEX1(Kdr)+")t";
        ...
        if (traceLevel > 2)    traceInfo += "ntISR = "+AES256Util.toHEX1(ta);

            // InvSubBytes(state) into a using inverse S-box Si
        for (i = 0; i < BLOCK_SIZE; i++) a[i] = Si[ta[i] & 0xFF];
        if (traceLevel > 2)    traceInfo += "ntISB = "+AES256Util.toHEX1(a);

        // AddRoundKey(state) into ta
        for (i = 0; i < BLOCK_SIZE; i++)    ta[i] = (byte)(a[i] ^ Kdr[i]);
        if (traceLevel > 2)    traceInfo += "nt AK = "+AES256Util.toHEX1(ta);
    ...
    if (traceLevel > 1)    traceInfo += "n  R"+numRounds+" (Key = "+AES256Util.toHEX1(Kdr)+")t";
    ...
    if (traceLevel > 2)    traceInfo += "ntISR = "+AES256Util.toHEX1(a);
    ...
    public void setKey(byte[] key) {
	    ...
	    traceInfo = "";            // reset trace info
	    if (traceLevel > 0) traceInfo = "setKey(" + AES256Util.toHEX1(key) + ")n";
	    ...
	    if (traceLevel > 3) {
	        traceInfo += "  Encrypt Round keys:n";
	            for(r=0;r<numRounds+1;r++) traceInfo += "  R"+r+"t = "+AES256Util.toHEX1(Ke[r])+"n";
	        traceInfo += "  Decrypt Round keys:n";
	            for(r=0;r<numRounds+1;r++) traceInfo += "  R"+r+"t = "+AES256Util.toHEX1(Kd[r])+"n";
	    }
    }
    ...
    byte [] key    = AES256Util.hex2byte(hkey);
    byte [] plain    = AES256Util.hex2byte(hplain);
    byte [] cipher    = AES256Util.hex2byte(hcipher);
    byte [] result;

    Aes256 testAES = new Aes256();    // create new AES instance to test triple
    ...
    if (Arrays.equals(result, cipher))
        ...
        System.out.print("Test Failed. Result was "+AES256Util.toHEX(result)+"n");
	...
	public byte[] _cryptAll(byte[] data, int mode)  {
        Aes256 aes = this;
        byte[] text = null;
        int modLength = data.length % 16;
        int nPaddValue = modLength == 0 ? 0 : (16 - modLength);
		byte padding = (byte)nPaddValue;
         
        text = new byte[data.length + nPaddValue];
        
        System.arraycopy(data, 0, text, 0, data.length);

        for(int i=data.length; i<text.length; i++) {
            text[i] += padding;
        }
        
            
        int nParts = (int)text.length/16;
        byte[] res = new byte[text.length];
        byte[] partByte = new byte[16];
        
        for(int p=0; p<nParts; p++) {
        	
            System.arraycopy(text, p*16, partByte, 0, partByte.length);
    ...
    public byte[] getFixedKey() {
		int nCount = 0;
		byte[] seed = {0,0,0,0};
        byte[] byKey = {0x33, (byte)0xff, (byte)0xa1,0x16, 0x78, 
                     (byte)0x56, (byte)0xf4, 0x23, 0x78, 0x34, 
                     (byte)0xaf, 0x33, 0x65, (byte)0x9f, 0x76, 0x13}; 
        
        byte keyBuff[];
    	keyBuff = new byte[32];
        
        if(nCount>0) {
            System.arraycopy(seed, 0, keyBuff, 0, seed.length);
        } else{
            System.arraycopy(byKey, 0, keyBuff, 0, byKey.length);
            return keyBuff;
        }
        
        byte[] getKey = new byte[32];
        ...
        try {
            ...
            for(int i=0;i<nCount;i++) {
                ...
                byte[] hash=md.digest();    
                System.arraycopy(hash, 0, getHash, 0, hash.length);
            }

            System.arraycopy(getHash, 0, getKey, 0, 16);
        }
    ...
    public static Aes256 shared(){
    	if(aes256 == null){
    		aes256 = new Aes256();
    		aes256.setKey(aes256.getFixedKey());
    	}
    	return aes256;
    }
    ...
    public String decrypt(String input){
    	if(input == null)
    		return input;
    	byte[] dec = Decrypt(AES256Util.hex2byte(input));
    	return getDataToString(Base64.decodeBase64(dec));
    }
    
    public String encrypt(String input){
    	if(input == null)
    		return input;
    	byte[] enc = Encrypt(Base64.encodeBase64(input.getBytes()));
		return AES256Util.toHEX1(enc);
    }
    
    public static String getDataToString(byte[] arryByte) {

        if(null == arryByte || 0 == arryByte.length) {
            return "";
        }
        
        try {
            int len = 0;
            for(len=0; len<arryByte.length; len++) {
                 if (0 == arryByte[len]) {
                      break;
                 }
            }
